...
namespace Tizen.FH.FamilyBoard
{
    ...
    public class FamilyBoardMain : ILifecycleObserver
    {
        ...
        {
            if (e.TapGesture.State == Gesture.StateType.Clear)
            {
                HideDeleteButton();
        ...
        private void ShowSelectedImage()
        {
            ...
            for (int i = 0; i < imageCount; i++)
            {
                ...
                if (ImageManager.Instance.GetDataItemType(i) == ImageManager.ItemType.PHOTO)
                {
                    ...
                    Tizen.Log.Error("MYLOG", "select style : " + style);
                    ...
                    case 2:
                        {
                            HideMenu();
                            mRootViewTapGestureDetector.Detach(mRootView);
                            FamilyBoardApplication.Instance.CreateView("Stickers");
                            break;
                        }
                    case 4:
                        {
                            HideMenu();
                            mRootViewTapGestureDetector.Detach(mRootView);
                            FamilyBoardApplication.Instance.CreateView("Text");
                            break;
                        }
                    default:
                        break;
    ...
    internal class MenuButtonsItemView : View
    {
        ...
        private bool OnMoreItemTouchEvent(object source, View.TouchEventArgs e)
        {
            ...
            FamilyBoardApplication.Instance.CreateView("Background Image");
