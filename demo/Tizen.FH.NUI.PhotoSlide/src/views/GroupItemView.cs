...
namespace NUIPhotoSlide
{
    class GroupItemView : View
    {
        ...
        {
            Tizen.Log.Error("MYLOG", "Init Items");
            float color = 1.0f;
            float scale = 1.0f;
            float ratio = 0.12f;
            ...
            for(int i = 0; i < imageItemList.Count; i++)
            {
                ...
                Tizen.Log.Error("MYLOG", "scale :" + scale);
                scale -= ratio;
                color -= 0.15f;
            }
        ...
        public GroupItemView(GroupLayerView parentGroup, int page, List<ImageItemData> data, List<Item> itemList, bool isScaleAnimation = false, VideoItem vitem = null)
        {
            ...
            Item prev = null;
            View mainImage = null;
            Tizen.Log.Error("MYLOG", " data length : " + data.Count);
            for (int idx = 0; idx < data.Count; idx++)
            {
                ...
                {
                    item.StartPosition = new Position((int)(mainImage.PositionX + (mainImage.SizeWidth / 2)) - (int)(item.SizeWidth / 2), (int)(mainImage.PositionY + (mainImage.SizeHeight / 2)), 0.0f);
                }
        ...
        private void AnimationFinished(object source, EventArgs e)
        {
            ...
            item_event_checker++;
            Tizen.Log.Error("MYLOG", "play next - " + item_event_checker);
            Tizen.Log.Error("MYLOG", "play len - " + length);
